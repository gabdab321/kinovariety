...
const HomePage = () => {
    ...
    return (
        <div className="HomePage">
            ...
            {films?.items ? <List title="Фільми" films={filterFilms(films.items).slice(0, 10)}/> : <Loader style={{margin: "40px auto"}}/>}

            {isErrorSeries ? <ErrorWarning/> : ""}
            {series?.items ? <List title="Серіали" films={filterFilms(series.items).slice(0, 10)}/> : <Loader style={{margin: "40px auto"}}/>}
        </div>
    );
};
